...
function errorMessage(error,status=0){
 ...
 if(spanishMessages.has(message))return message;
 if(code==='signup_disabled'||code==='user_not_found'||/signups not allowed|user not found/i.test(message))return 'Este correo no está registrado para acceder al evento. Comprueba la dirección o contacta con la organización.';
 ...
 if(status===403||code==='42501'||/permission denied|access denied|not authorized/i.test(message))return 'No tienes permiso para realizar esta acción. Contacta con la organización.';
 ...
 return 'No se ha podido completar la operación. Vuelve a intentarlo. Si el problema continúa, contacta con la organización.';
}
